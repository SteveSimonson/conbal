...
test('page profile keeps readable content after oversized inline scripts', async () => {
  const environment = environmentForPage();
  const originalFetch = globalThis.fetch;
  const oversizedHtml = `<html><head><title>Events that scale</title><script>${'window.payload='.repeat(180000)}</script></head><body><main><h1>Events that scale</h1><p>${'Readable event planning guidance for teams. '.repeat(45)}</p></main></body></html>`;
  globalThis.fetch = async input => {
    assert.equal(String(input), pageUrl);
    return new Response(oversizedHtml, { status: 200, headers: { 'content-type': 'text/html; charset=utf-8' } });
  };
  try {
    ...
    assert.equal(response.status, 200);
    assert.equal(result.profile.truncated, true);
    assert.equal(result.profile.title, 'Events that scale');
    assert.equal(result.profile.slots.length, 1);
    assert.equal(result.profile.headings[0], 'Events that scale');
  } finally { globalThis.fetch = originalFetch; }
});
