...
- Use this exact header: `title,slug,size,html,css`
...
The five required headers are case-insensitive when imported, but generators should use the exact lowercase names and order below. Each required header may appear only once. Extra columns are accepted by the importer but should not be generated because Conbal ignores them.
...
```text
Create [NUMBER] Conbal content balloons as a UTF-8 CSV.

Business/site: [BUSINESS NAME AND SITE PURPOSE]
Audience: [AUDIENCE]
Goal: [PROMOTION, SIGNUP, ANNOUNCEMENT, PRODUCT EDUCATION, ETC.]
...
Leave the header unquoted. Quote every data-field value, double every quote inside a field, use unique valid slugs, scope every CSS selector under a unique slug-derived root class, and do not invent facts, prices, deadlines, coupon codes, image sources, or URLs.
```
...
5. Conbal validates every row before it writes any row. If any row is invalid or any slug conflicts in the selected site, nothing is imported.
...
- The first row is exactly `title,slug,size,html,css`.
